with fact_assetcosts as (
    select
        ...
        installation_date,
        {{ surrogate_key('client_id','installation_full_month') }} as installation_clientperiod_key,
        ...
        decomissioned_date,
        {{ surrogate_key('client_id','decomissioned_full_month') }} as decommissioned_clientperiod_key,       
        building_item_number,
        client_building_item_number,
        material_quantity,
        description,
        unit_price,
        client_replacement_responsibility_ind,
        practical_life,
        practical_life_adjustment,
        mean_service_life,
        asset_comments,
        building_item_criticality,
        replacement_cost,
        adjusted_replacement_cost,
        field_replacement_estimate_cost
    from {{ ref('stg_fact_assetcosts') }}
)

, fact_assetcosts_lookup as (
    select 
        ...
        installation_date,
        coalesce(i_cp.clientperiod_key,0) as installation_clientperiod_key,
        practicallife_replacement_date,
        ...
        decomissioned_date,
        coalesce(dc_cp.clientperiod_key,0) as decommissioned_clientperiod_key,       
        building_item_number,
        client_building_item_number,
        material_quantity,
        description,
    ...
    left join {{ ref('dim_clientperiod') }} i_cp 
        on f.installation_clientperiod_key = i_cp.clientperiod_key 
    ...
    left join {{ ref('dim_clientperiod') }} dc_cp 
        on f.decommissioned_clientperiod_key = dc_cp.clientperiod_key 
)

select * from fact_assetcosts_lookup
